...
\subsection{Available Datasets}
 For training a DL agents a huge amount of data is required, which is why a dataset is required. We looked for publicly available datasets that can be used for training DL decision-making agents. Unfortunately, all of them can only be applied for testing a feasibility of social behaviour personalisation models. Because the datasets contain visual data we categorised them into two groups, depending on the perspective of the camera form which the video/pictures were recorded. The following datasets contain data recorded from the robot perspective (the human partner/'s is/are directly interacting with the robot):
\begin{itemize}
	\item AIR-Act2Act~\cite{Ko2021}: 100 subjects, 10 actions, 5000 samples, 4 modalities (depth, skeleton, RGB)
	\item NTU RGB+D~\cite{Liu2020,Shahroudy_2016_CVPR}: 106 subjects, 26 actions, 8276 samples, 3 modalities (depth, skeleton, RGB)
...
And the following datatsets contain data recorded from the perspective of the third person (two people are interacting with each other and the robot is a passive observer):
\begin{itemize}
...
Unfortunately, only certain datasets are easily available online, namel
Easy to access:
\begin{itemize}
	\item AIR-Act2Act: \cite{Ko2021}
	\item AUTH UAV Gesture (gesture recognition) Dataset:~\cite{patrona2021overview,Liu2020,Perera_2018_ECCV_Workshops}
