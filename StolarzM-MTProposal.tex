...

{
	...
	\begin{longtable}{|p{6cm}||p{6cm}|}
		...
		\hline
		\multirow{3}{\linewidth}{Enabling the robot to adapt the activity difficulty and social behaviour} & Providing the DL network with the visual data and an extra input vector (similarly to~\cite{Qureshi2018,Belo2022,Hijaz2021}) containing the learning activity information \\
		\cline{2-2}
		& Maintenance of an adequate action space~\cite{stolarz2022learningbased,stolarz2022personalisedrobot,tsiakas2018task} enabling the robot to give a feedback to the user and change an activity difficulty\\
		\hline
		\multirow{3}{\linewidth}{Preventing the robot from making socially unacceptable decisions} & Pretraining on the manually collected dataset~\cite{stolarz2022learningbased,stolarz2022personalisedrobot} \\
		\cline{2-2}
		& Applying learning from guidance approach (similarly to~\cite{senft2017supervised}) where the robot's actions can be corrected by the supervisor before being executed
		\\
		\hline
...
%Notes:
% discuss ALL the deficits in the related work section
