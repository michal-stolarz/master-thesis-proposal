...
In another work~\cite{liu2018learning}, the authors face the problem of learning a social behaviour from human-human interaction data for a robot acting as a shopkeeper. The robot is supposed to perform not only reactive actions, but also the proactive ones, namely, the robot should continue the interaction even if the user does not say anything. The contribution of this work is that (i) no human annotation for the behaviour model training is needed, (ii) the model can generate verbal (dialogue) and nonverbal (locomotion and proxemics) actions, and (iii) the robot is capable of learning from the interaction history. The used behaviour model is a DNN with an attention layer. The input of the system consists of the features describing an interaction and its history (e.g. customer's/shopkeeper's location, spoken utterances). The action space contains spoken utterances with corresponding target formation (the robot moves when target and current formation is different). The algorithm was evaluated (i) with the collected dataset (where human coders were labeling a prediction as socially acceptable or not) and (ii) in a structured user study. Although, the model showed an  improvement in comparison to the much simpler model, the reported behaviour correctness was relatively low (around 62\%). Another deficit is that the proposed algorithm is not designed to work on the raw sensory data (i.e. camera image).
