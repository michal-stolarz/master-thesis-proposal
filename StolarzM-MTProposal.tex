...
The problem of low accuracy was faced in the follow-up paper~\cite{carpio2019learning}, where the authors managed to model the temporal rules governing the interaction with the use of Interval Temporal Bayesian Networks (ITBN)~\cite{zhang2013modeling}. ITBN is a probabilistic approach for modeling complex activities through temporal relations between individual simple events. The proposed framework is called TR-LFD and consists of two layers. In the first layer, called Spatial Reasoning Layer (SPL), there are two CNN's that extract spatial features from an audio spectrogram and optical flow data. In the second layer, called Temporal Reasoning Layer (TRL), there is ITBN that is used for action selection based on the features extracted in the previous layer. The authors performed evaluation both on the dataset and in the real-life interaction (conducted in the laboratory environment). Based on the comparative evaluation (in real-life), the authors showed that TR-Lfd outperformed DR-Lfd, by obtaining accuracy above 90\%. When it comes to the dataset evaluation, the accuracy was around 84\%. The deficits ...
